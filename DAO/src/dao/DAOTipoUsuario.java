...
public class DAOTipoUsuario extends conexion{
    ...
    public List<TipoUsuario> listar() throws Exception {
        ...
        try {
            this.conectar(false);
            rs = this.ejecutarOrdenDatos("SELECT codigo_TU, descripcion_TU, estado_TU "
                    + " FROM tipousuario TU "
                    + "ORDER BY descripcion_TU");
    ...
    public void cambiarEstado(int id) throws Exception {
        String sql = "UPDATE TipoUsuario SET estado_TU = 0 WHERE codigo_TU= "+id+";";
        try {
            this.conectar(true);
            this.ejecutarOrden(sql);
            this.cerrar(true);
